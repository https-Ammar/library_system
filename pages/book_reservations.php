...
if (isset($_GET['delete'])) {
    ...
    try {
        $getBook = $mysqli->prepare("SELECT book_id, status, quantity, amount_paid FROM BookReservations WHERE reservation_id = ? AND deleted_at IS NULL");
        ...
        $amount_paid = $bookData['amount_paid'];

        $stmt = $mysqli->prepare("UPDATE BookReservations SET deleted_at = NOW(), amount_paid = 0, amount_due = 0 WHERE reservation_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_reservation_id'])) {
    ...
    try {
        ...
        if (($status === 'cancelled' || $status === 'returned') && !in_array($old_status, ['cancelled', 'returned'])) {
            ...
            $stmt = $mysqli->prepare("UPDATE BookReservations SET amount_paid = 0, amount_due = 0 WHERE reservation_id = ?");
            $stmt->bind_param("i", $reservation_id);
            $stmt->execute();
            $stmt->close();
        } elseif (($old_status === 'cancelled' || $old_status === 'returned') && !in_array($status, ['cancelled', 'returned'])) {
...
$filter_status = isset($_GET['status']) ? $mysqli->real_escape_string($_GET['status']) : '';
...
if (!empty($filter_status)) {
    $reservations_query .= " AND br.status = '$filter_status'";
}
...
SELECT 
    ...
    SUM(CASE WHEN br.status = 'cancelled' THEN 1 ELSE 0 END) AS cancelled_count,
...
SELECT t.teacher_id, t.name, 
       ...
       SUM(CASE WHEN br.status NOT IN ('cancelled','returned') THEN br.amount_paid ELSE 0 END) AS total_amount,
       SUM(CASE WHEN br.status NOT IN ('cancelled','returned') THEN br.amount_due ELSE 0 END) AS total_due
FROM Teachers t
...
<body x-data="{ page: 'saas', loaded: true, darkMode: false, stickyMenu: false, sidebarToggle: false, scrollTop: false, isTaskModalModal: false }" x-init="darkMode = JSON.parse(localStorage.getItem('darkMode')); $watch('darkMode', value => localStorage.setItem('darkMode', JSON.stringify(value)))" :class="{'dark bg-gray-900': darkMode === true}">
                    ...
                    <div class="col-span-12">
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6 xl:grid-cols-4">
                            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                                ...
                                <p class="text-theme-sm text-gray-500 dark:text-gray-400">إجمالي المدرسين</p>
                                <div class="mt-3 flex items-end justify-between">
                                    <div>
                                        <h4 class="text-title-sm font-bold text-gray-800 dark:text-white/90"><?= $teachers_stats->num_rows ?></h4>
                                    </div>
                                    <div class="flex items-center gap-1">
                                        <span class="flex items-center gap-1 rounded-full bg-success-50 px-2 py-0.5 text-theme-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-500">+</span>
                                    </div>
                                </div>
                            </div>
                            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                                ...
                                <p class="text-theme-sm text-gray-500 dark:text-gray-400">إجمالي الحجوزات</p>
                                <div class="mt-3 flex items-end justify-between">
                                    <div>
                                        <h4 class="text-title-sm font-bold text-gray-800 dark:text-white/90"><?= $reservations->num_rows ?></h4>
                                    </div>
                                    <div class="flex items-center gap-1">
                                        <span class="flex items-center gap-1 rounded-full bg-error-50 px-2 py-0.5 text-theme-xs font-medium text-error-600 dark:bg-error-500/15 dark:text-error-500">-</span>
                                    </div>
                                </div>
                            </div>
                            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                                ...
                                <p class="text-theme-sm text-gray-500 dark:text-gray-400">إجمالي المبالغ</p>
                                <div class="mt-3 flex items-end justify-between">
                                    <div>
                            ...
                            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                                <a href="?status=pending">
                                    ...
                                    <p class="text-theme-sm text-gray-500 dark:text-gray-400">الطلبات المعلقة</p>
                                    <div class="mt-3 flex items-end justify-between">
                                        <div>
                                            <h4 class="text-title-sm font-bold text-gray-800 dark:text-white/90"><?= $stats['pending_count'] ?></h4>
                                        </div>
                                        <div class="flex items-center gap-1">
                                            <span class="flex items-center gap-1 rounded-full bg-warning-50 px-2 py-0.5 text-theme-xs font-medium text-warning-600 dark:bg-warning-500/15 dark:text-warning-500">+</span>
                                        </div>
                                    </div>
                                </a>
                            </div>
                            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                                <a href="?status=approved">
                                    <div class="mb-6 flex h-[52px] w-[52px] items-center justify-center rounded-xl bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400">
                                        <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM12 20C7.58 20 4 16.42 4 12C4 7.58 7.58 4 12 4C16.42 4 20 7.58 20 12C20 16.42 16.42 20 12 20ZM10 15.5L7.5 13L8.91 11.59L10 12.67L14.09 8.58L15.5 10L10 15.5Z"></path>
                                        </svg>
                                    </div>
                                    <p class="text-theme-sm text-gray-500 dark:text-gray-400">الطلبات الموافق عليها</p>
                                    <div class="mt-3 flex items-end justify-between">
                                        <div>
                                            <h4 class="text-title-sm font-bold text-gray-800 dark:text-white/90"><?= $stats['approved_count'] ?></h4>
                                        </div>
                                        <div class="flex items-center gap-1">
                                            <span class="flex items-center gap-1 rounded-full bg-success-50 px-2 py-0.5 text-theme-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-500">+</span>
                                        </div>
                            ...
                            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                                <a href="?status=cancelled">
                                    <div class="mb-6 flex h-[52px] w-[52px] items-center justify-center rounded-xl bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400">
                                        <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM12 20C7.58 20 4 16.42 4 12C4 7.58 7.58 4 12 4C16.42 4 20 7.58 20 12C20 16.42 16.42 20 12 20ZM15.5 8.5L12 12L15.5 15.5L14.09 16.91L10.59 13.41L7.09 16.91L5.67 15.5L9.17 12L5.67 8.5L7.09 7.09L10.59 10.59L14.09 7.09L15.5 8.5Z"></path>
                                        </svg>
                                    </div>
                                    <p class="text-theme-sm text-gray-500 dark:text-gray-400">الطلبات الملغاة</p>
                                    <div class="mt-3 flex items-end justify-between">
                                        <div>
                                            <h4 class="text-title-sm font-bold text-gray-800 dark:text-white/90"><?= $stats['cancelled_count'] ?></h4>
                                        </div>
                                        <div class="flex items-center gap-1">
                                            <span class="flex items-center gap-1 rounded-full bg-error-50 px-2 py-0.5 text-theme-xs font-medium text-error-600 dark:bg-error-500/15 dark:text-error-500">-</span>
                                        </div>
                            ...
                            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
                                <a href="?status=returned">
                                    <div class="mb-6 flex h-[52px] w-[52px] items-center justify-center rounded-xl bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-400">
                                        <svg class="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM12 20C7.58 20 4 16.42 4 12C4 7.58 7.58 4 12 4C16.42 4 20 7.58 20 12C20 16.42 16.42 20 12 20ZM7 12H17L13 8L14.41 6.59L19.83 12L14.41 17.41L13 16L17 12Z"></path>
                                        </svg>
                                    </div>
                                    <p class="text-theme-sm text-gray-500 dark:text-gray-400">الطلبات المرتجعة</p>
                                    <div class="mt-3 flex items-end justify-between">
                                        <div>
                                            <h4 class="text-title-sm font-bold text-gray-800 dark:text-white/90"><?= $stats['returned_count'] ?></h4>
                                        </div>
                                        <div class="flex items-center gap-1">
                                            <span class="flex items-center gap-1 rounded-full bg-error-50 px-2 py-0.5 text-theme-xs font-medium text-error-600 dark:bg-error-500/15 dark:text-error-500">-</span>
                                        </div>
                    ...
                    <div class="col-span-12 mt-6">
                        <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
                            <div class="flex flex-col gap-5 px-6 mb-4 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">حجوزات الكتب</h3>
                                </div>
                                <div class="flex items-center gap-3">
                                    <form method="GET" action="" class="flex">
                                        <div class="relative">
                                            ...
                                            <input type="text" name="search" placeholder="بحث..." value="<?= isset($_GET['search']) ? htmlspecialchars($_GET['search']) : '' ?>" class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-10 w-full rounded-lg border border-gray-300 bg-transparent py-2.5 pr-4 pl-[42px] text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden xl:w-[300px] dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                                        </div>
                                    </form>
                                </div>
                            </div>
                                ...
                                <table class="min-w-full">
                                    <thead class="border-gray-100 border-y bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
                                        <tr>
                                            ...
                                            <th class="px-6 py-3 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">اسم الطالب</p>
                                                </div>
                                            </th>
                                            <th class="px-6 py-3 whitespace-nowrap">
                                                <div class="flex items-center">
                                            ...
                                            <th class="px-6 py-3 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">الإجمالي</p>
                                                </div>
                                            </th>
                                            <th class="px-6 py-3 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">المدفوع</p>
                                                </div>
                                            </th>
                                            <th class="px-6 py-3 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">الباقي</p>
                                                </div>
                                            </th>
                                            <th class="px-6 py-3 whitespace-nowrap">
                                                <div class="flex items-center">
                                            ...
                                            <th class="px-6 py-3 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">الإيصال</p>
                                                </div>
                                            </th>
                                            <th class="px-6 py-3 whitespace-nowrap">
                                                <div class="flex items-center">
                                            ...
                                            <th class="px-6 py-3 whitespace-nowrap">
                                                <div class="flex items-center justify-center">
                                                    <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">حفظ</p>
                                                </div>
                                            </th>
                                            <th class="px-6 py-3 whitespace-nowrap">
                                                <div class="flex items-center justify-center">
                                                    <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">حذف</p>
                                                </div>
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                        <?php if ($reservations && $reservations->num_rows > 0):
                                            while ($row = $reservations->fetch_assoc()):
                                                ...
                                                <tr>
                                                    <td class="px-6 py-3 whitespace-nowrap">
                                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400"><?= htmlspecialchars($row['order_number']) ?></p>
                                                    </td>
                                                    <td class="px-6 py-3 whitespace-nowrap">
                                                        <div class="flex items-center">
                                                            <div class="flex items-center gap-3">
                                                                <div class="flex items-center justify-center w-10 h-10 rounded-full bg-brand-100">
                                                                    <span class="text-xs font-semibold text-brand-500"><?= $row['reservation_id'] ?></span>
                                                                </div>
                                                                <div>
                                                                    <span class="text-theme-sm mb-0.5 block font-medium text-gray-700 dark:text-gray-400"><?= htmlspecialchars($row['student_name']) ?></span>
                                                                    <span class="text-gray-500 text-theme-sm dark:text-gray-400"><?= htmlspecialchars($row['grade_name']) ?></span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td class="px-6 py-3 whitespace-nowrap">
                                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400">
                                                            ...
                                                            ?>
                                                            <?= !empty($phone) ? '<a href="https://wa.me/' . $phoneWithCode . '" target="_blank">' . htmlspecialchars($row['student_phone']) . '</a>' : '-' ?>
                                                        </p>
                                                    </td>
                                                    <td class="px-6 py-3 whitespace-nowrap text-center">
                                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400"><?= $row['quantity'] ?></p>
                                                    </td>
                                                    <td class="px-6 py-3 whitespace-nowrap">
                                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400"><?= htmlspecialchars($row['teacher_name']) ?></p>
                                                    </td>
                                                    <td class="px-6 py-3 whitespace-nowrap">
                                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400"><?= htmlspecialchars($row['book_title']) ?></p>
                                                    </td>
                                                    <td class="px-6 py-3 whitespace-nowrap text-end">
                                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400"><?= number_format($row['book_price'], 2) ?></p>
                                                    </td>
                                                    <td class="px-6 py-3 whitespace-nowrap text-end">
                                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400"><?= number_format($total_price, 2) ?></p>
                                                    </td>
                                                    <td class="px-6 py-3 whitespace-nowrap text-end">
                                                        <form method="post" class="flex items-center gap-2 justify-end">
                                                            ...
                                                            <input type="number" name="amount_paid" step="0.01" min="0" max="<?= $total_price ?>" value="<?= $row['amount_paid'] ?>" class="dark:bg-dark-900 h-8 w-24 rounded-lg border border-gray-300 bg-transparent px-2 py-1 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                                                        </form>
                                                    </td>
                                                    <td class="px-6 py-3 whitespace-nowrap text-end">
                                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400"><?= number_format($row['amount_due'], 2) ?></p>
                                                    </td>
                                                    <td class="px-6 py-3 whitespace-nowrap text-center">
                                                        <p class="text-gray-700 text-theme-sm dark:text-gray-400"><?= $row['reservation_date'] ?></p>
                                                    </td>
                                                    <td class="px-6 py-3 whitespace-nowrap text-center">
                                                        <?php if (!empty($row['receipt_image'])): ?>
                                                            <a href="<?= $row['receipt_image'] ?>" target="_blank" class="text-blue-500 hover:underline text-gray-700 text-theme-sm dark:text-gray-400">عرض الإيصال</a>
                                                        <?php else: ?>
                                                    ...
                                                    <form method="post" enctype="multipart/form-data" class="flex items-center gap-2">
                                                        <td class="px-6 py-3 whitespace-nowrap text-center">
                                                            <input type="hidden" name="edit_reservation_id" value="<?= $row['reservation_id'] ?>">
                                                            <select name="status" required class="dark:bg-dark-900 h-8 w-24 rounded-lg border border-gray-300 bg-transparent px-2 py-1 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                                                        ...
                                                        <td class="px-6 py-3 whitespace-nowrap text-center">
                                                            <button type="submit" class="btn btn-primary btn-sm text-gray-700 text-theme-sm dark:text-gray-400">
                                                    ...
                                                    <td class="px-6 py-3 whitespace-nowrap">
                                                        <div class="flex items-center justify-center gap-3">
                                                            <a href="?delete=<?= $row['reservation_id'] ?>" onclick="return confirm('تأكيد الحذف؟');" class="cursor-pointer hover:fill-error-500 dark:hover:fill-error-500 fill-gray-700 dark:fill-gray-400">
                                            ...
                                            <?php endwhile; else: ?>
                                            <tr>
                                                <td colspan="15" class="text-center py-4 text-gray-500 dark:text-gray-400">لا توجد بيانات للعرض</td>
                                            </tr>
                                        <?php endif; ?>
                                    </tbody>
                                </table>
